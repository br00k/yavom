...
<? $divid = uniqid(); ?>
  <? if ($p->getUser()) :?>
     ...
     <?php echo  $s?image_tag("icons/delete",array(
		"id"=>"manager_del_button",
		"class"=>"button",
		"title"=>__('Delete manager'),
		"onClick"=>'parent.location="'.url_for("show/deleteManager?s_id=".$s->getId().'&p_id='.$p->getId()).'"',
		)):"" ?>
     <? //= $i?$i->getId():"" ?>
     <? echo $me?'</span>':"" ?>
     <br>
...
<? if ($i) :?>
		...
		autoOpen: false,
        	width: 500,
		title: '<?php echo __('Remove Manager')?>',
                buttons: {
